import React from "react";
import { projectsData } from "../../data/Data";
import { useState } from "react";

function Projects() {
  const [sortBy, setSortBy] = useState("default");
  const [category, setCategory] = useState("");
  return (
    <div>
      <div className="flex justify-center items-center gap-4 mt-10 px-10 ">
        <div className="grow">
          <h1> لیست پروژه ها </h1>
        </div>
        <ProjectsStatus />
        <ProjectsSort
          sortBy={sortBy}
          projectsData={projectsData}
          onSort={e => setSortBy(e.target.value)}
        />
        <ProjectsCategory
          category={category}
          onCategory={e => setCategory(e.target.value)}
          projectsData={projectsData}
        />
      </div>
      <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-14 px-10 ">
        <ProjectsTable
          category={category}
          onCategory={e => setCategory(e.target.value)}
          projectsData={projectsData}
          sortBy={sortBy}
          onSort={e => setSortBy(e.target.value)}
        />
      </div>
    </div>
  );
}
export default Projects;

function ProjectsTable({ sortBy, projectsData, setCategory }) {
  let sortedProject = projectsData;
  if (sortBy === "earliest")
    sortedProject = [...projectsData].sort(
      (a, b) => new Date(a.createdAt) - new Date(b.createdAt)
    );
  if (sortBy === "latest")
    sortedProject = [...projectsData].sort(
      (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
    );
  if (sortBy === "default") sortedProject = projectsData;
  return ProjectsList(sortedProject);
}

function ProjectsSort({ sortBy, onSort }) {
  return (
    <div className="">
      <select
        value={sortBy}
        onChange={onSort}
        id=""
        className=" w-32 text-black rounded-xl py-2 text-sm px-2"
      >
        <option value="default" className="" selected>
          مرتب سازی (پیش فرض)
        </option>
        <option value="earliest" className="">
          مرتب سازی (جدیدترین)
        </option>
        <option value="latest" className="">
          مرتب سازی (قدیمی ترین)
        </option>
      </select>
    </div>
  );
}
function ProjectsStatus() {
  return (
    <div className="flex py-1 rounded-lg items-center gap-2">
      <span>وضعیت: </span>
      <ul class="flex gap-1 text-sm rounded-xl p-1 bg-white w-36">
        <li class="">
          <a
            href="#"
            class="inline-block px-2 py-1 text-white bg-blue-600 rounded-lg active"
            value="all"
          >
            همه
          </a>
        </li>
        <li class="me-2">
          <a
            href="#"
            class="inline-block px-2 py-1 rounded-lg hover:text-gray-900 hover:bg-gray-300 text-black "
            value="open"
          >
            باز
          </a>
        </li>
        <li class="me-2">
          <a
            href="#"
            class="inline-block px-2 py-1 rounded-lg hover:text-gray-900 hover:bg-gray-300 text-black "
            value="close"
          >
            بسته
          </a>
        </li>
      </ul>
    </div>
  );
}
function ProjectsCategory({ category, onCategory }) {
  return (
    <div className="">
      <select
        value={category}
        onChange={onCategory}
        name=""
        id=""
        className="w-32 text-black rounded-xl py-2 text-sm px-2"
      >
        <option value=""> دسته بندی ها</option>
        <option value="design-ui/ux">طراحی (ui/ux) </option>
        <option value="web development"> برنامه نویسی وب</option>
      </select>
    </div>
  );
}
function ProjectsList(projects) {
  return (
    <table class="w-full text-sm text-center  text-gray-500 dark:text-gray-400">
      <thead class="text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        <tr>
          <th scope="col" class="px-2 py-3">
            ردیف
          </th>
          <th scope="col" class="px-2 py-3">
            عنوان پروژه
          </th>
          <th scope="col" class="px-2 py-3">
            بودجه (تومان)
          </th>
          <th scope="col" class="px-2 py-3">
            ددلاین
          </th>
          <th scope="col" class="px-2 py-3">
            وضعیت
          </th>
          <th scope="col" class="px-2 py-3">
            عملیات
          </th>
        </tr>
      </thead>
      <tbody>
        {projects.map((project, index) => (
          <tr
            key={project._id}
            class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700"
          >
            <th
              scope="row"
              class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white"
            >
              {String(index + 1)}
            </th>
            <td class="px-6 py-4">{project.title}</td>
            <td class="px-6 py-4">{project.budget}</td>
            <td class="px-6 py-4">
              {new Date(project.deadline).toLocaleDateString("fa-IR")}
            </td>
            <td class="px-6 py-4">
              {project.status === "OPEN" ? (
                <span className="bg-green-700 text-white rounded-xl px-4 py-1 text-xs">
                  باز
                </span>
              ) : (
                <span className="bg-rose-500 text-white rounded-xl px-2 py-1 text-xs">
                  بسته
                </span>
              )}
            </td>
            <td class="px-6 py-4 flex justify-center ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke-width="1.5"
                stroke="currentColor"
                class="size-6 cursor-pointer"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z"
                />
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
                />
              </svg>
            </td>
          </tr>
        ))}
      </tbody>
    </table>
  );
}
